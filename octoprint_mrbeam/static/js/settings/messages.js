...
$(function () {
    function MessagesViewModel(parameters) {
        ...
        self.checkRestriction = function(restrictions){
            ...
            if (restrictions.channels && restrictions.channels.length > 0
                && !restrictions.channels.map(function(x){ return x.trim().toUpperCase()}).includes(MRBEAM_SW_TIER)) {
                return false
            }

            // version
            if (restrictions.version && restrictions.version.length > 0
                && !restrictions.version.map(function(x){ return x.trim().toUpperCase()}).includes(BEAMOS_VERSION)) {
                return false
            }
            ...
            // not_first_run
            if (restrictions.not_first_run && CONFIG_FIRST_RUN) {return false}
    ...
    OCTOPRINT_VIEWMODELS.push([
        ...
        ["#settings_plugin_mrbeam_messages"]
    ]);
});
